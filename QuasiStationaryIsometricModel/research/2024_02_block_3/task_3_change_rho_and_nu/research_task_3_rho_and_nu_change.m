...
minValue3 = min(data3(:, 2:end-1), [], 'all') - 5;
maxValue3 = max(data3(:, 2:end-1), [], 'all') + 5;

filename4 = 'research_out/nu_profile.csv';
data4 = readtable(filename4);
data4 = dlmread(filename4, ';', 0, 0);
minValue4 = min(data4(:, 2:end-1), [], 'all') - 0.1e-5;
maxValue4 = max(data4(:, 2:end-1), [], 'all') + 0.1e-5;
...
subplot(4, 1, 4);
plot(km, data4(1, 2:end-1), 'Color', 'b');
xlabel('Труба, км');
ylabel('Вязкость, сСт');
title(['t = ' num2str(data4(1,1)) ', с']);
newXLimit = [0, 100];
xlim(newXLimit);
newYLimit = [minValue4, maxValue4];
ylim(newYLimit);
...
for i = 2:size(data(:,1))
    ...
    subplot(4, 1, 4);
    plot(km, data4(i, 2:end-1), 'Color', 'b');
    xlabel('Труба, км');
    ylabel('Вязкость, сСт');
    title(['t = ' num2str(data4(i,1)) ', с']);
    newXLimit = [0, 100];
    xlim(newXLimit);
    newYLimit = [minValue4, maxValue4];
    ylim(newYLimit);
...
filename = 'task_3_rho_and_nu_change.gif';
